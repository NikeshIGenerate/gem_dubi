      ...
      child: Row(
        ...
        children: <Widget>[
          if (messageOptions.showOtherUsersAvatar)
            Opacity(
              opacity: !isOwnMessage && (!isNextSameAuthor || isBeforeDateSeparator) ? 1 : 0,
              child: getAvatar(),
            ),
          if (!messageOptions.showOtherUsersAvatar) const Padding(padding: EdgeInsets.only(left: 8)),
          GestureDetector(
            onLongPress: messageOptions.onLongPressMessage != null
                ? () => messageOptions.onLongPressMessage!(message)
                : null,
            onTap:
                messageOptions.onPressMessage != null ? () => messageOptions.onPressMessage!(message) : null,
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  if (message.text.isNotEmpty)
                    Padding(
                      padding: !isOwnMessage ? const EdgeInsets.only(left: 46) : EdgeInsets.zero,
                      child: TextContainer(
                        messageOptions: messageOptions,
                        currentUser: currentUser,
                        message: message,
          ...
          if (!messageOptions.showCurrentUserAvatar) const Padding(padding: EdgeInsets.only(left: 10))
        ],
      ),
    );
  }
